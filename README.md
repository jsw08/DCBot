...
| LastFM | These commands show what song you or a lastfm user is currently playing | None | Everywhere | INSERT PC |
| `/fm np` | Gets the currently playing song of either your lastfm username or the provided one. | - `username`: string; overwrites the lastfm username. | Everywhere | INSERT PC |
| `/fm set` | Sets your lastfm user. It alsoc ckecks if the provided username exists.| - `username`: REQUIRED string; the username that will be set in the database. | Everywhere | INSERT PC |
